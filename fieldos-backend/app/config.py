...
class Settings:
    ...
    REAL_SMS_ENABLED: bool = os.getenv("REAL_SMS_ENABLED", "false").strip().lower() in ("1", "true", "yes", "on")
    SMS_PROVIDER_ALLOWLIST: str = os.getenv("SMS_PROVIDER_ALLOWLIST", "log")
    ...
    SMS_REQUIRE_APPROVED_TEMPLATE: bool = os.getenv("SMS_REQUIRE_APPROVED_TEMPLATE", "true").strip().lower() in ("1", "true", "yes", "on")
    SMS_REQUIRE_CONSENT: bool = os.getenv("SMS_REQUIRE_CONSENT", "true").strip().lower() in ("1", "true", "yes", "on")
    SMS_REQUIRE_SUPPRESSION_CHECK: bool = os.getenv("SMS_REQUIRE_SUPPRESSION_CHECK", "true").strip().lower() in ("1", "true", "yes", "on")
