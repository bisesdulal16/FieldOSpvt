...
async def test_memory_same_nonce_same_request_rejected(monkeypatch):
    ...
    monkeypatch.setattr(settings, "N8N_REPLAY_TTL_SECONDS", 300)
...
async def test_memory_different_nonce_accepted_and_nonce_only_after_ttl(monkeypatch):
    monkeypatch.setattr(settings, "N8N_REPLAY_STORE", "memory")
    monkeypatch.setattr(settings, "N8N_REPLAY_TTL_SECONDS", 1)
    ...
    replay_module._MEMORY_REPLAY_CACHE[first.key] = datetime.utcnow() - timedelta(seconds=2)
...
@pytest.mark.parametrize("ttl", [0, -1, 86401, "not-int"])
@pytest.mark.asyncio
