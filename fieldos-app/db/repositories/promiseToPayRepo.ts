...
export async function getActivePromises(): Promise<any[]> {
  return query("SELECT * FROM promise_to_pay WHERE status = 'active'");
}

export async function getPromisesByClient(clientId: number): Promise<any[]> {
  return query('SELECT * FROM promise_to_pay WHERE client_id = ?', [clientId]);
}

export async function markPromiseFulfilled(id: number): Promise<void> {
  await mutate(
    "UPDATE promise_to_pay SET status = 'fulfilled' WHERE id = ?",
    [id]
  );
}

export async function getPendingSyncPromises(): Promise<any[]> {
  return query("SELECT * FROM promise_to_pay WHERE sync_status = 'pending'");
}

export async function markPromiseSynced(id: number): Promise<void> {
  await mutate(
    "UPDATE promise_to_pay SET sync_status = 'synced' WHERE id = ?",
    [id]
  );
}
